...
async def chat_init(body: str):
    ...
    if existing_core_anchor:
        ...
        # 确保 isApply 为 2（可能上次初始化完成后没更新成功）
        try:
            from sqlalchemy import text
            async with async_session_local() as session:
                result = await session.execute(
                    text("SELECT isApply FROM llm_user WHERE id = :llm_id"),
                    {"llm_id": llm_id}
                )
                row = result.fetchone()
                if row and row[0] != 2:
                    await session.execute(
                        text("UPDATE llm_user SET isApply = 2 WHERE id = :llm_id"),
                        {"llm_id": llm_id}
                    )
                    await session.commit()
                    logger.info(f"【激活完成-补偿】llmId={llm_id}, isApply已更新为2")
        except Exception as e:
            logger.error(f"【激活完成-补偿】llmId={llm_id}, 更新isApply失败: {e}", exc_info=True)
        return
    ...
    try:
        ...
        async with async_session_local() as session:
            ...
            await session.commit()
        logger.info(f"【激活完成】llmId={llm_id}, isApply已更新为2")
    except Exception as e:
